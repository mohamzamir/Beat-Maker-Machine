...
function state(s) {
  ...
  if (s === "init") {
    ...
    function updateLightPosition(e) {
      ...
      smoothX = mouseX * smoothingFactor + (1 - smoothingFactor) * smoothX;
      smoothY = mouseY * smoothingFactor + (1 - smoothingFactor) * smoothY;
    }

    function animateLightPosition() {
      currentX += (smoothX - currentX) * smoothingFactor;
      currentY += (smoothY - currentY) * smoothingFactor;
      app.universe.light.position.x = currentX;
      app.universe.light.position.y = currentY;
      requestAnimationFrame(animateLightPosition);
    }

    wrapper.addEventListener('mousemove', updateLightPosition);
...
class Application {
  ...
  render() {
    ...
    let gradient = this.context.createRadialGradient(this.universe.light.position.x, this.universe.light.position.y, 0.5 * settings.spotlightRadius, this.universe.light.position.x, this.universe.light.position.y, 0);
...
class Box {
  ...
  getShadowCorners(light) {
    ...
    for (var i = 0; i < this.sides; i++) {
      ...
      // The distance calculation is now based on the distance to the light source, and multiplied by a factor that gets smaller as the distance increases.
      let shadowDistanceFactor = Math.max(0, 1 - dist / settings.spotlightRadius);
      let x = light.position.x + dx * shadowDistanceFactor * 20;
      let y = light.position.y + dy * shadowDistanceFactor * 20;

      // and add this corner to our array of shadowCorners
